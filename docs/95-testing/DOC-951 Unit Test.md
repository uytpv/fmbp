...
- Sử dụng thư viện test mặc định của Dart/Flutter.
- Sử dụng thư viện **Mocktail** hoặc **Mockito** để giả lập (mock) các kết nối mạng và kết nối cơ sở dữ liệu bên ngoài.
- Quy tắc đặt tên file test: Trùng tên file code chính và thêm hậu tố _test.dart (ví dụ: budget_service_test.dart).
