...
## 2. Scope
Áp dụng cho cấu trúc các bộ sưu tập chính: users, amilies, ecipes, pantry_items, shopping_lists, udgets.
...
- Cấu trúc thư mục con (Sub-collections) để phân nhóm dữ liệu gia đình (ví dụ: amilies/{familyId}/pantry_items).
...
## 6. Business Rules
- Mọi tài liệu con trong nhánh amilies đều phải có thuộc tính amilyId để phục vụ bảo mật phân quyền.

## 7. Data Model
- Sơ đồ chi tiết cấu trúc Firestore Collections.

## 8. Flow
- Người dùng cập nhật số lượng nguyên liệu -> Ứng dụng ghi đè lên tài liệu amilies/{familyId}/pantry_items/{itemId} -> Firestore kích hoạt sự kiện đồng bộ.
