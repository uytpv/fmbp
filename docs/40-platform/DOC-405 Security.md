...
- Triển khai Firestore Security Rules đảm bảo: Một người dùng chỉ được đọc/ghi dữ liệu của nhóm gia đình (familyId) mà họ là thành viên chính thức.
...
## 8. Flow
- App gửi yêu cầu đọc dữ liệu -> Firestore Security Rules kiểm tra: request.auth.uid != null và resource.data.familyId == request.auth.token.familyId -> Nếu hợp lệ -> Trả về dữ liệu; Ngược lại -> Chặn và báo lỗi Permission Denied.
